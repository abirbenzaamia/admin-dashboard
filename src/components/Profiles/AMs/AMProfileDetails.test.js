...
import AMPProfile from './AMProfileDetails'
...
it("displays the trending gifs received from Giphy API",  () => {
    ...
    const { getByAltText } = render(<AMPProfile user={user[0]}></AMPProfile>)
     ...
     expect(screen.getByDisplayValue(user[0].num_tel)).toBeInTheDocument();
     expect(screen.getByDisplayValue(user[0].email)).toBeInTheDocument();
